  ...
  mutate(class_order2 = 
           plyr::mapvalues(class_order, 
                           ...
                           to = c("FC1-2", 
                                  ...
                                  "FC7",
                                  "FC8"))) %>%
  mutate(class_order2 = as.factor(class_order2))
```
...
p_diagnosis_all <- myDF.fc %>%
  plotCat("diagnosis", class = "class_order2") 

p_sex_all <- myDF.fc %>%
  plotCat("sex", class = "class_order2")

p_age_all <- myDF.fc %>%
  ggplot(aes(x = class_order2, y = age)) +
...
p_mlr_all <- myDF.fc %>%
  mutate(class_order2 = relevel(class_order2, ref = "FC3-6")) %>%
  mlrPlot(var = c("diagnosis", "age", "sex"), class = "class_order2") 
```
...
p_mlr <- myDF.fc %>%
  filter(probmax > 0.5) %>%
  mutate(class_order2 = relevel(class_order2, ref = "FC3-6")) %>%
  mlrPlot(var = c("diagnosis", "age", "sex"), class = "class_order2") # , "baseline_fc"
...
p_extent_uc <- myDF.fc %>%
  ...
  plotCat("Extent", class = "class_order2")
...
```{r}
myDF.crp %>%
  mutate(class_aux = ifelse(class_order %in% c("CRR6"), class_order, "Other")) %>%
  group_by(class_aux, diagnosis) %>%
  summarise(count = n()) %>%
  mutate(percentage = (count / sum(count) * 100)) %>%
  knitr::kable()
